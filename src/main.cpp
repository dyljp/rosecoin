...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // RoseCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "RoseCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
        ...
        int rand = generateMTRandom(seed, 11999);
        int rand1 = 0;
        int rand2 = 0;
		int day = 0;
        ...
        if(nHeight < 500000)    
        {
        		if(nHeight <= 1440)
    			{
    				if(nHeight == 1)
					{
						nSubsidy = MAX_MONEY * 0.02;
						return nSubsidy + nFees;
					}
					else
					{
						nSubsidy = 2.14 * (1 + rand) * COIN;
					}
    			}
				else
				{
        ...
        else if(nHeight < 1000000)      
        {
                ...
                seed = hex2long(cseed);
                rand1 = generateMTRandom(seed, 5999);
				// Bonus day				
				day = nHeight / 1440 + 1;
				if(day % 14 == 0)
				{
					nSubsidy = 2.14 * (1 + rand1) * COIN;
				}
				else
				{
        ...
        else if(nHeight < 1500000)      
        {
                ...
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand2 = generateMTRandom(seed, 2999);
				// Bonus day	
				day = nHeight / 1440 + 1;
				if(day % 14 == 0)
				{
					nSubsidy = 2.14 * (1 + rand2) * COIN;
				}
				else
				{
		...
		else
		{
				...
				if(day % 14 == 0)
				{
					nSubsidy = 2.14 * nSubsidy;
				}
				else
				{
					nSubsidy = nSubsidy * 1.214;
				}
		}
...
static const int64 nTargetTimespan = 30 * 60; // RoseCoin: every 30 minutes
static const int64 nTargetSpacing = 60; // RoseCoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // RoseCoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal rosecoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "RoseCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "In Rose We Love! Oh, man";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x8a004935b94db25628184b33a9bfb53a4bb22b192589755c5e193f41c297ab94"));
		...
		//// debug print  GenesisBlock Found
        block.print();
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // RoseCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
